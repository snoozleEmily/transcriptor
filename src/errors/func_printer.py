...
def _sanitize_path(path: str) -> str:
    ...
    try:
        if isinstance(path, str) and (os.path.sep in path or any(path.lower().endswith(ext) for ext in ('.mp4', '.avi', '.mov', '.mkv'))):
    ...
    if kwargs:
        ...
        for key, value in kwargs.items():
            if key == "config_params":
                ...
                display = _sanitize_path(value) if isinstance(value, str) else str(value)
