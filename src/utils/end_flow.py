...
class EndFlow:
    ...
    @catch_errors
    def process_video(self, video_path: str, config_params: Optional[Dict[str, Any]] = None, **kwargs) -> str:
        ...
        if not revised_text:
            print(get_func_call(self.process_video, (video_path,), {"config_params": config_params, **kwargs}))
    ...
    def _save_result(self, text: str, source_filename: str = "") -> str:
        ...
        try:
            if save_path.lower().endswith('.pdf'):
                ...
                if not success:
                    raise FileError(code=ErrorCode.FILE_ERROR, message="PDF creation failed")
            else:
