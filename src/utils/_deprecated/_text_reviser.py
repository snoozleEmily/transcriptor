...
class AdvancedTextReviser:
    ...
    def _is_gibberish(self, text):
        ...
        tech_matches = sum(1 for w in word_tokenize(text) 
        ...
        return (
            (verb_count == 0 and len(text) > 15) or
            (entity_count/len(word_tokenize(text)) > 0.4) or
            (tech_matches/len(self.technical_terms) < 0.2 if self.technical_terms else False) or
            (not summary or summary[0].text.count(' ') < 5)
        )
    ...
    def _enforce_grammar_rules(self, text):
        ...
        for i in range(2, len(pos_tags)):
            if pos_tags[i-2][1].startswith('NN') and \
               pos_tags[i-1][1].startswith('NN') and \
               pos_tags[i][1].startswith('NN'):
    ...
    def _restore_proper_nouns(self, text, whisper_result):
        ...
        if not whisper_result:
            return text
        original_words = [w['word'] for seg in whisper_result['segments']
